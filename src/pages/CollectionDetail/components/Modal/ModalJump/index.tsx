...
export const ModalJump = (props: any) => {
  ...
  const nft: any = useGetNft(
    props.nftsJump?.nftsDetail?.identifier.toString(),
    Number(props.nftsJump?.nftsDetail?.nonce),
    props.openModalJump
  );
      ...
      <div ref={ModalRef} className='backgroundStakeModal_Collection_Jump'>
        <div className='modalStakeModal_Collection'>
          <div className='contentStakeModal_Collection'>
            ...
            >
              {props?.nftsJump?.collectionRewards
                .filter(
                  (item: any) =>
                    item.pool_id !==
                      props.nftsJump?.collectionReward?.pool_id &&
                    (Number(item.nonce) === 0 ||
                      Number(item.nonce) ===
                        Number(props.nftsJump?.nftsDetail?.nonce))
                )
                    ...
                    <div key={index}>
                      <RowJumpPool
                        ...
                        getCollectionInformations={
                          props.getCollectionInformations
                        }
                        inforamtion={item}
                      />
                    </div>
            ...
            <div>
              <ActionJumpNFT
                Nft_id={props.nftsJump?.nftsDetail?.id}
                pool_id={PoolSelected}
                disabled={
                  props.nftsJump?.nftsDetail?.id === undefined ||
